...
- (NSString *)getAccountNameForConnection:(GBConnection *)connection
{
	return [[NSArray arrayWithObjects:connection.type, @" - ", connection.name, @"(", connection.uuid, @")", nil] componentsJoinedByString:@""];
}
...
- (NSString *)getPasswordForConnection:(GBConnection *)connection
{
    OSStatus code;
    UInt32 passwordLength;
    char *passwordData;
	
	NSString *account = [self getAccountNameForConnection:connection];
...
- (void)setPasswordForConnection:(GBConnection *)connection to:(NSString *)password
{
    OSStatus code;
    UInt32 passwordLength;
    char *passwordData;
...
- (void)deletePasswordForConnection:(GBConnection *)connection
{
	OSStatus code;
	UInt32 passwordLength;
	char* passwordData;
